...
class PlateNumber extends StatelessWidget {
  const PlateNumber({super.key, required this.plateNumber});
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: isKurdistan(plateNumber)
          ? KurdistanPlate(plateNumber: plateNumber)
          : isIraq2024(plateNumber)
          ? Iraq2024Plate(plateNumber: plateNumber)
          : isIraq2008(plateNumber.trim())
          ? Iraq2008Plate(plateNumber: plateNumber)
          : Expanded(
              child: Row(
                children: [
                  DecoratedBox(
                    decoration: BoxDecoration(
                      border: Border.all(
                        color:
                            context.textTheme.bodyLarge!.color ?? Colors.black,
                      ),
...
class Iraq2008Plate extends HookConsumerWidget {
  const Iraq2008Plate({super.key, required this.plateNumber});
  final String plateNumber;
...
class Iraq2024Plate extends StatelessWidget {
  const Iraq2024Plate({super.key, required this.plateNumber});
...
class KurdistanPlate extends StatelessWidget {
  const KurdistanPlate({super.key, required this.plateNumber});
